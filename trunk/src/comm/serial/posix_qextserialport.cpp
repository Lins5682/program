...
bool Posix_QextSerialPort::open(OpenMode mode)
{
    ...
    if (!isOpen()) {
        ...
        //disable qDebug("DEV<SERIAL>: trying to open file %s.", port.toStdString().c_str());
        if (Posix_File->open(QIODevice::ReadWrite|QIODevice::Unbuffered)) {
            //disable qDebug("DEV<SERIAL>: opened file succesfully.");
            /*set open mode*/
            QIODevice::open(mode);
            ...
            setStopBits(Settings.StopBits);
            setFlowControl(Settings.FlowControl);
            // code_ok, replace by zfx20161223
            //setTimeout(Settings.Timeout_Sec, Settings.Timeout_Millisec);
            setTimeout(Settings.Timeout_Millisec);
            tcsetattr(Posix_File->handle(), TCSAFLUSH, &Posix_CommConfig);
        } else {
            //disable qDebug("DEVSERIAL: Could not open FILE, Error code : %d", Posix_File->error());
        }
    }
    UNLOCK_MUTEX();
    return isOpen();
